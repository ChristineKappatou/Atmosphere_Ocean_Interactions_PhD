...
def make_filelist_cmor(
    ...
    # very quick fix
    if (
        self.name == "ACCESS-ESM1-5"
        and self.expid == "ssp585"
        and self.realiz in ["r2i1p1f1", "r3i1p1f1"]
    ):
        print("Quick fix for ACCESS-ESM1-5 SSP5-8.5 files:!")
        print(fnames)
        fnames.reverse()
    if (
        self.name == "NorESM2-LM"
        and self.expid in ["esm-ssp245", "esm-ssp534-over"]
        #and self.realiz in ["r10i1p1f1"]
    ):
        print("Removed extra file from %s %s"%(self.expid, self.realiz))
        fnames = [f for f in fnames if not f.endswith('202001-210012.nc')]
        fnames = [f for f in fnames if not f.endswith('225001-225012.nc')]
    if (
        self.name == "NorESM2-MM"
        and self.expid in ["ssp585-snow"]
    ):
        fnames = [f for f in fnames if not f.endswith('209101-209912.nc')]
    if fnames:
...
def make_filelist_raw(
    ...
    *,
    ...
    pattern = dict(
        atmos=f"{expid}/atm/hist/{expid}.cam.h0a.*.nc",
        ocean=f"{expid}/ocn/hist/{expid}.*.hm.*.nc",
        hamocc=f"{expid}/ocn/hist/{expid}.*.hbgcm.*.nc",
...
def make_filelist_raw_highres(
    ...
    *,
    ...
    # This is not working for mid-month files!
    # if month is not None:
    #     """
    #     Usually, high-frequency output is separated and stored in 5 files for each year
    #     year-01-01-00000.nc, year-03-15-00000, year-05-27-00000.nc, year-08-08-00000, year-10-20-00000
    #     so it is really just monthly files and January, this will work. Thus, if not January, read the whole year and
    #     subtract data from the correct month after the lazy reading.
    #     """
    #     monthpaths = [
    #         path for path in paths if date_from_path_highres(path).month == month
    #     ]
    #     print(monthpaths)
    #     if monthpaths:
    #         paths = monthpaths
    #     else:
    #         f"\nPLEASE NOTE! no files found for {expid} between {first_year}--{last_year} for month nr.: {month}"
    #         f"Paths include the files for the whole year"
    #         paths = paths
    #     assert (
    #         paths
    #     ), f"no files found for {expid} between {first_year}--{last_year} for month nr.: {month}"
